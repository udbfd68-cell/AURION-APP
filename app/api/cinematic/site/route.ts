...
import { cinematicSiteSchema } from '@/lib/api-schemas';
...
export async function POST(req: Request) {
  ...
  let body;
  try { body = cinematicSiteSchema.parse(await req.json()); } catch { return Response.json({ error: 'Invalid JSON body' }, { status: 400 }); }
